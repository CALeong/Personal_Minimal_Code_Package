...
import scipy.sparse
from Fundamental.vacancies import general_q3_hamiltonian_equal_sublattice_vacancy_density_add_on_top
import pickle
...
def run_KPM_ADOS_parallel_vacancies_q3(n_jobs, sparse_tbham, eigval_min, eigval_max,
    ...
    with parallel_config(backend='loky'):
        Parallel(n_jobs=n_jobs)(
            delayed(run_routine)
            (rescale_operator_unity_window(general_q3_hamiltonian_equal_sublattice_vacancy_density_add_on_top(sparse_tbham,
                                                                                                              pval,
                                                                                                              nval,
                                                                                                              vac_density),
            ...
            save_name,
            save_index)
            for (vac_density, save_index) in zip(vac_density_list, save_index_list)
        )

    metadata_dict = {
        'pval': pval,
        'nval': nval,
        'Vacancy density list': vac_density_list,
